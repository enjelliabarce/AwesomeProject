...
// Titik pusat geofence (Warung Empal Bu Warno)
...
export default function TrackingMapScreen({ route }) {
  ...
  useEffect(() => {
    const peminjamanRef = ref(database, 'peminjaman'); //listener untuk semua motor yang dipinjam
    ...
    const unsubscribe = onValue(peminjamanRef, snapshot => {
      ...
      Object.entries(val).forEach(([id, data]) => {
        ...
        const { latitude: lat, longitude: lng, accuracy: acc, speed } = data.lokasi;
        ...
        // Simpan state lokal
        activeMotors.current[id] = { lat, lng, acc: acc ?? 0, speed: speed ?? 0, namaMotor, platMotor };
